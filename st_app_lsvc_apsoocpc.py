...
st.title("Dr. Z.C.M.")
st.title('AI predicts satisfaction of online clinical probation course')  # 算法名称 and XXX

COL_INPUT = [
    'Sex', '"Year in medical school,"', 'Time per lecture',
    'Lecture frequency per week', 'Opportunity to take quizzes',
    'Opportunity to submit assignments', 'Opportunity to give oral presentations',
    'Opportunity to observe physicians’ practice', 'Opportunity to practice clinical skills',
    'Opportunity to participate in interprofessional meetings', 'Opportunity to interact with physicians',
    'Frequency of technical Internet-related problems',  # 'Satisfaction level'
]
...
def do_processing():
    ...
    # 分割数据
    X_train, X_test, y_train, y_test = do_split_data(pocd, random_state=2)
    ...
    lsvc = LinearSVC(penalty='l2', loss='squared_hinge', dual=True, tol=0.0001, C=1.0, multi_class='ovr',
                     fit_intercept=True, intercept_scaling=1, class_weight=None, verbose=0, random_state=1,
                     max_iter=1000)
